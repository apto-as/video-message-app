...
import tempfile
import os
from core.constants import MAX_LARGE_AUDIO_SIZE_BYTES
...
async def clone_voice(
    ...
    try:
        ...
        # ファイルサイズチェック（100MB制限）
        if len(audio_data) > MAX_LARGE_AUDIO_SIZE_BYTES:
...
async def _background_clone_task(
    ...
    try:
        ...
        try:
            ...
            # 一時ファイルの削除失敗は無視（既に削除されている可能性）
            logger.debug(f"一時ファイル削除失敗（問題なし）: {e}")
